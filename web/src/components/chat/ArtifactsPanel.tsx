...
import { CalendarIcon, CodeIcon, FileIcon, FolderIcon, UploadIcon, Loader2Icon, PlayIcon, ArrowLeftIcon, SearchIcon } from 'lucide-react';
...
export function ArtifactsPanel() {
  ...
  const { files, uploadFile, isUploading } = useWorkspaceFiles();
  ...
  const artifactsByMessage = useMemo(() => {
    ...
    const completeMessages = messages.filter(m => m.role === 'assistant' && !m.streaming);
                          ...
                          {artifact.language === 'mermaid' ? (
                            ...
                            <div className="max-h-[200px] overflow-y-auto">
                              <CodeBlock
                                ...
                                className="rounded-none border-none text-[10px] leading-tight overflow-hidden [&_pre]:whitespace-pre-wrap [&_pre]:wrap-break-word"
                              />
                            </div>
                          )}
                ...
                {!activeConversationId ? (
                  ...
                  <div className="space-y-1 overflow-auto min-h-0">
                    {filteredFiles.map((file, idx) => (
                      ...
                      >
                        {file.is_dir ? <FolderIcon className="size-4 text-blue-400" /> : <FileIcon className="size-4 text-gray-400" />}
                        <div className="flex-1 min-w-0">
                          <div className="font-medium truncate">{file.name}</div>
                          <div className="text-[10px] text-muted-foreground flex gap-2">
                            <span>{file.mod_time ? format(new Date(file.mod_time), 'MMM d, h:mm a') : '-'}</span>
